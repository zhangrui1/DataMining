 ...
 *
 */
@WebServlet(name = "ChangeKiki", urlPatterns = { "/ChangeKiki" })
@MultipartConfig(fileSizeThreshold = 5000000, maxFileSize = 10000000)
public class ChangeKiki extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			List<String[]> userDataList = ValdacUtilites
					.createInstanceCheckDataLength(csvData,ValdacConfig. Length_Kiki);
			assert userDataList != null;

			// kikisystemテーブルに保存
			ValdacUtilites.resetDB(conn, ValdacConfig.TABLENAME_Kiki); // DBの初期化
			ValdacUtilites.importDataKiki(conn, userDataList,
					ValdacConfig.TABLENAME_Kiki); // 機器システムテーブル
