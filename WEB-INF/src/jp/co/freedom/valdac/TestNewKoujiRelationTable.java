 ...
 *
 */
@WebServlet(name = "TestNewKoujiRelationTable", urlPatterns = { "/TestNewKoujiRelationTable" })
@MultipartConfig(fileSizeThreshold = 5000000, maxFileSize = 10000000)
public class TestNewKoujiRelationTable extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			// 工事履歴の 工事IDがないデータMap
			List<ValdacUserDataDto> allNoMatchKoujiKikiDataList = new ArrayList<ValdacUserDataDto>();
			List<ValdacUserDataDto> allMatchKoujiKikiDataList = ValdacUtilites
					.getKoujiKikiIdDataForStep(conn,Integer.parseInt(StartName));

				for (ValdacUserDataDto userData : allMatchKoujiKikiDataList) {
...
//						System.out.println("該機器ID：" + userData.kikiIDOld);
					...
					if((userData.koujiID==null) ||(userData.KikiSysId==null) || (userData.kikiID==null) ){
						allNoMatchKoujiKikiDataList.add(userData);
					}
				}
...
//			}
			...
			if ("match".equals(ModeName)){
				if (!ValdacUtilites.downLoadTenkenRireki(request, response,
						ValdacConfig.OUTPUT_TENKENKIKI, allMatchKoujiKikiDataList,
						ValdacConfig.DELIMITER)) {
					System.out.println("Error: Failed download CSV file");
				}
			}else {
				if (!ValdacUtilites.downLoadTenkenRireki(request, response,
						ValdacConfig.OUTPUT_TENKENKIKI_NO, allNoMatchKoujiKikiDataList,
						ValdacConfig.DELIMITER)) {
					System.out.println("Error: Failed download CSV file");
				}
			}
