 ...
 *
 */
public class JecaValidator extends Validator {
	 ...
	 *            不明マーク
	 */
	public JecaValidator(String mark) {
		super();
	...
	public void validate(UserDataDto userdata, String validationType) {
		...
		if ("1".equals(validationType)) {
			...
			boolean V_ADDR5 = !StringUtil.containWildcard(
					userdata.cardInfo.V_ADDR5, this.undecipherableMark);
			...
			boolean V_NAME2 = util.isPreEntry(userdata) ? StringUtil
					.isNotEmpty(userdata.cardInfo.V_NAME2) : true;
			// 連絡先情報
			...
			boolean V_ADDR = true;
			if (util.isPreEntry(userdata) || util.isAppEntry(userdata)) {
				V_ADDR = V_ADDR1 && V_ADDR2 && V_ADDR3 && V_ADDR4;
			} else if (util.isGroup(userdata)) {
				V_ADDR = V_ADDR1;
			}
			...
			// 連絡先情報の欠落フラグ
			boolean addressIsValid = JecaUtil
					.isOversea((JecaUserDataDto) userdata) ? V_ADDR2 : V_ZIP
					&& V_ADDR;
