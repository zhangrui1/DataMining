 ...
 *
 */
public class IpUtil extends Util {
	...
	public static List<MesagoUserDataDto> getAllAppointedDayData(Connection conn)
		...
		while (rs.next()) {
			...
			// 環境依存文字の存在性を確認
			if (IpConfig.EXECUTE_VALIDATE_MODEL_DEPENDENCE
	...
	public static boolean downLoad(HttpServletRequest request,
		...
		// アンケート項目
		if (IpConfig.OUTPUT_ENQUETE_RESULTS_FLG) {
		...
		// 統計情報
		for (int nDay = 1; nDay <= IpConfig.DAYS.length; nDay++) {
		...
		// SLXコード
		Map<String, MesagoSLXDto> slxMap = IpSLXCodeUtil.getSLXMap(conn,
				IpConfig.EXHIBITION_NAME);
			...
			if (visitFlg) {
				if (StringUtil.isNotEmpty(userdata.timeByRfid)) {
					...
					cols.add(StringUtil.enquote(StringUtil.normalizedDateStr(
							IpConfig.YEAR, IpConfig.MONTH,
							userdata.cardInfo.V_DAY, IpConfig.HOUR)));
				} else if (StringUtil
	...
	public static List<String> outputEnqueteDataForAppointedday(
		...
		Map<String, MesagoSLXDto> contactPostType = IpSLXCodeUtil.contactPostType;
		Map<String, MesagoSLXDto> contactPostGroup = IpSLXCodeUtil.contactPostGroup;
		Map<String, MesagoSLXDto> accOfStaff = IpSLXCodeUtil.accOfStaff;
		...
		// 出展検討
		cols.add(StringUtil.enquote(StringUtil
				.isNotEmpty(userdata.questionInfo.V_Q8) ? IpConfig.MARK : ""));
		return cols;
	}
	...
	public static List<String> outputEnqueteDataForPreentry(List<String> cols,
			MesagoUserDataDto userdata) {
		Map<String, String> accOfStaffForPreRegist = IpSLXCodeUtil.accOfStaffForPreRegist;
		// 業種番号
		cols.add(StringUtil.enquote(""));
		// 業種区分
		cols.add(StringUtil.enquote(MesagoUtil
		...
		if (!outputStatisticalData) {
			...
			if (userdata.visitFlgs == null) {
				...
				if (StringUtil.isNotEmpty(imagePath)) {
					...
					boolean flg4 = StringUtil.find(imagePath, month + days[3]);
					userdata.visitFlgs = new boolean[4];
					userdata.visitFlgs[0] = flg1;
					userdata.visitFlgs[1] = flg2;
					userdata.visitFlgs[2] = flg3;
					userdata.visitFlgs[3] = flg4;
				}
			}
			if (userdata.visitFlgs != null) {
