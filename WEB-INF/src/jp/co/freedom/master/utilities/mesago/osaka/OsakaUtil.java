package jp.co.freedom.master.utilities.mesago.osaka;
 ...
 *
 */
public class OsakaUtil extends Util {
	...
	private static TICKET_TYPE getTicketType(UserDataDto userdata) {
		 ...
		 * else if (util.isVIPInvitation(userdata)) {// VIP招待券ユーザーである場合 return
		 * TICKET_TYPE.vip_invitation; }
	...
	public static List<MesagoUserDataDto> createInstance(List<String[]> csvData) {
		...
		for (int nIndex = 0; nIndex < csvData.size(); nIndex++) {
			...
			if (row.length == 4) {// RFID番号の行
				...
				dataDto.id = row[2]; // バーコード番号
				dataDto.timeByRfid = row[1]; // タイムスタンプ
			} else {
				// 読み飛ばし
				continue;
			}
	...
	public static List<MesagoUserDataDto> getAllAppointedDayData(Connection conn)
		...
		while (rs.next()) {
			...
			if (StringUtil.isNotEmpty(rs.getString("V_SH"))) {
			...
			cardInfo.V_OVERSEA = MesagoUtil
					.normalize(rs.getString("V_OVERSEA")); // 海外住所フラグ
			...
			questionInfo.V_Q8 = MesagoUtil.normalize(rs.getString("V_Q8")); // 出展検討

			// /* プレス情報 */
			// cardInfo.V_P_NAME =
			// MesagoUtil.normalize(rs.getString("V_P_NAME"));
			...
			// 環境依存文字の存在性を確認
			if (OsakaConfig.EXECUTE_VALIDATE_MODEL_DEPENDENCE
	...
	public static List<MesagoUserDataDto> getAllPreRegistData(Connection conn,
		...
		if (visitFlg) {
			...
			sql = "SELECT * FROM preentry WHERE IMAGE_PATH='1';";
//			List<String> conditions = new ArrayList<String>();
//			for (String day : OsakaConfig.DAYS) {
...
//			sql = sql + StringUtil.concatWithDelimit(" and ", conditions);
		...
		while (rs.next()) {
			...
			questionInfo.V_Q7 = EnqueteUtil.convertSingleAnswer(MesagoUtil
					.normalize(rs.getString("V_Q7"))); // 来場動機
			questionInfo.V_Q7_other = MesagoUtil.normalize(rs.getString("V_Q7_FA")); // 来場動機(その他FA)
			questionInfo.V_Q8 = MesagoUtil.normalize(rs.getString("V_Q8")); // 出展検討
	...
	public static boolean downLoad(HttpServletRequest request,
		...
		header.add(StringUtil.enquote("VIP"));
		...
		// アンケート項目
		if (OsakaConfig.OUTPUT_ENQUETE_RESULTS_FLG) {
		...
		// 統計情報
		for (int nDay = 1; nDay <= OsakaConfig.DAYS.length; nDay++) {
		...
		// SLXコード
		Map<String, MesagoSLXDto> slxMap = OsakaSLXCodeUtil.getSLXMap(conn,
				OsakaConfig.EXHIBITION_NAME);
		...
		for (MesagoUserDataDto userdata : userDataList) {
			...
			if (visitFlg) {
				if (StringUtil.isNotEmpty(userdata.timeByRfid)) {
					...
					cols.add(StringUtil.enquote(StringUtil.normalizedDateStr(
							OsakaConfig.YEAR, OsakaConfig.MONTH,
							userdata.cardInfo.V_DAY, OsakaConfig.HOUR)));
				} else if (StringUtil
						.isNotEmpty(((MesagoCardDto) userdata.cardInfo).V_IMAGE_PATH)) { // 画像パスから取得
					String date = ((MesagoCardDto) userdata.cardInfo).V_IMAGE_PATH
							.substring(4, 8);
					cols.add(StringUtil.enquote(date));
				} else {
					continue;
				}
			} else {
			...
			outputStatisticalData(conn, cols, userdata, outputStatisticalData,
					OsakaConfig.MONTH, OsakaConfig.DAYS);
	...
	public static List<String> outputEnqueteDataForAppointedday(
		...
		Map<String, MesagoSLXDto> contactPostType = OsakaSLXCodeUtil.contactPostType;
		Map<String, MesagoSLXDto> contactPostGroup = OsakaSLXCodeUtil.contactPostGroup;
		Map<String, MesagoSLXDto> accOfStaff = OsakaSLXCodeUtil.accOfStaff;
		...
		// 出展検討
		cols.add(StringUtil.enquote(StringUtil
				.isNotEmpty(userdata.questionInfo.V_Q8) ? OsakaConfig.MARK : ""));
		return cols;
	}
	...
	public static List<String> outputEnqueteDataForPreentry(List<String> cols,
			MesagoUserDataDto userdata) {
		Map<String, String> accOfStaffForPreRegist = OsakaSLXCodeUtil.accOfStaffForPreRegist;
		// 業種番号
		cols.add(StringUtil.enquote(""));
		// 業種区分
		cols.add(StringUtil.enquote(MesagoUtil
		...
		// 出展検討
		cols.add(StringUtil.enquote(MesagoUtil
				.normalizedEnqueteText8(userdata.questionInfo.V_Q8)));
		return cols;
	}
	...
	public static List<String> outputStatisticalData(Connection conn,
		...
		// 来場日
		if (StringUtil.isNotEmpty(userdata.timeByRfid)) {
			String visitedDay = userdata.timeByRfid.substring(6, 8);
		...
		}else if(StringUtil.isNotEmpty(userdata.cardInfo.V_IMAGE_PATH)) {
			String visitedDay = userdata.cardInfo.V_IMAGE_PATH.substring(6, 8);
			for (String day : MesagoConfig.DAYS) {
				if (day.equals(visitedDay)) {
					cols.add(StringUtil.enquote("T"));
				} else {
	 ...
	 * @throws SQLException
	 */
	public static Map<String, IpRfidDto> getAllRfidMap(Connection conn)
			throws SQLException {

		Map<String, IpRfidDto> allRfidData = new HashMap<String, IpRfidDto>();
		...
		while (rs.next()) {
			...
			IpRfidDto rfidDto = new IpRfidDto();
			int nIndex = -1;
			for (String day : OsakaConfig.DAYS) {
